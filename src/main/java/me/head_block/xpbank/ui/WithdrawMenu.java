...
public class WithdrawMenu {
	...
	public static void init() {
		INV_NAME = ChatColor.translateAlternateColorCodes('&',
				Main.instance.getConfig().getString("gui.withdraw-menu.name"));
		...
		if (slot < INV_SIZE && clicked.hasItemMeta() && clicked.getItemMeta().hasDisplayName()) {
			...
			if (percentage > 0.00) {
				...
				if (amount > Main.xps.get(p.getUniqueId().toString())) {
					p.sendMessage(ChatColor.RED + "You only have " + Main.xps.get(p.getUniqueId().toString())
							+ " xp in the bank");
					return;
				}
			...
			if (amount > 0) {
				...
				if (xpToAdd > Main.xps.get(p.getUniqueId().toString())) {
					p.sendMessage(ChatColor.RED + "You only have enough xp for "
							+ Utils.getMaxLevel(p, Main.xps.get(p.getUniqueId().toString())) + " levels");
					return;
				}
